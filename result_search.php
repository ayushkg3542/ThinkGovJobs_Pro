<?php
 ...
 $sql = "SELECT * FROM result_data LEFT JOIN blog_category ON result_data.category=blog_category.category_id WHERE result_title like '%$table_search%' or result_discription like '%$table_search%' ORDER BY date DESC limit $offset, $limit";
 ...
 if($result->num_rows > 0){
    while ($row = $result->fetch_assoc()) {
        ...
        $row['result_discription'] = substr($row['result_discription'],0 ,500) . '...';
...
<div class="container">
    <div class="row my-3">
        <div class="col-sm-8 col-md-8 col-12">
            ...
            <div class="card my-3">
                <div class="card-title p-3">
                    <a href="result-allnews.php?result_id=<?php echo $post['result_id']; ?>">
                        <h2><?php echo $post['result_title']; ?></h2>
                    </a>
                    <span><?php echo $post['date']?></span>
                </div>
                <div class="card-body">
                    <div class="card-text">
                        <p><?php echo $post['result_discription']; ?><a
                                href="result-allnews.php?result_id=<?php echo $post['result_id']; ?>">Read More</a></p>
                    </div>
                    <div class="link-part my-5">
                        <p><i class="bi bi-folder-fill"></i> <a href="result.php">Result</a></p>
                        <p><i class="bi bi-chat-left-dots-fill"></i> <a
                                href="result-allnews.php?result_id=<?php echo $post['result_id']; ?>">Leave a
                                comment</a></p>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
            <?php 
                $pagination ="SELECT * FROM result_data"; 
            ...
            <ul class="pagination pt-2 pb-5">
                ...
                <li class="page-item <?= ($i==$page) ? $active = 'active':'';?>">
                    <a href="result.php?page_no=<?= $i?>" class="page-link"><?= $i?></a>
                </li>
                <?php }?>
            </ul>
        </div>
                    ...
                    <div class="form-inline p-3">
                        <div class="input-group input-group-sm w-100">
                            <input type="text" name="table_search" id="search_key" class="form-control float-right"
                                placeholder="Search" value="<?= $table_search?>">
