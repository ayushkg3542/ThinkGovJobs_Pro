...
if(isset($_POST['delete_id'])){
    ...
    $delete_query = "DELETE FROM syllabus_data WHERE syllabus_id = '$delete_id'";
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-tools d-flex justify-content-center align-items-center" style="gap: 12px;">
                                ...
                                <a href="add_syllabus.php" class="btn btn-outline-secondary"><i class="bi bi-plus"></i>
                                    Add Syllabus Data</a>
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-4">
                                <div class="card-body">
                                    <h4 class="card-title"><strong><?php echo $post['syllabus_title']; ?></strong></h4>
                                    <!-- <p class="card-text"><?php echo $post['syllabus_discription']; ?></p> -->
                                    <p class="card-text">Date: <?php echo date('d F Y', strtotime($post['date'])); ?></p>
                                    <p>Author Name: <strong><?php echo $post['author_name']?></strong></p>
                                    <form action="" method="post" class="d-flex" style="gap: 10px"
                                        id="deleteBlog_<?php echo $post['syllabus_id']?>"
                                        onsubmit="return confirm('Are you sure you want to delete this blog?')">
                                        <a class="btn btn-outline-warning edit-news-btn"
                                            href="edit_blog.php?id=<?php echo $post['syllabus_id']; ?>"
                                            data-edit-id="<?php echo $post['syllabus_id']; ?>">Edit Blog</a>
                                        <input type="hidden" name="delete_id" value="<?php echo $post['syllabus_id']?>">
                                        <button class="btn btn-outline-danger delete-button"
                                            data-delete-id="<?php echo $row['syllabus_id']; ?>"><i
