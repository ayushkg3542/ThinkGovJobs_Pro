...
if(isset($_POST['delete_id'])){
    ...
    $delete_query = "DELETE FROM admitcard_data WHERE admitcard_id = '$delete_id'";
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-tools d-flex justify-content-center align-items-center" style="gap: 12px;">
                                ...
                                <a href="add_admit-card.php" class="btn btn-outline-secondary"><i
                                        class="bi bi-plus"></i>
                                    Add Admitcard Data</a>
                            </div>
                        </div>
                        <div class="row">
                            <?php foreach ($blogPosts as $post) { ?>
                            <div class="col-md-12 mb-4">
                                <div class="card-body searchable-items">
                                    <h4 class="card-title"><strong><?php echo $post['admitcard_title']; ?></strong></h4>
                                    <!-- <p class="card-text"><?php echo $post['admitcard_discription']; ?></p> -->
                                    ...
                                    <form action="" method="post" class="d-flex" style="gap: 10px;"
                                        id="deleteBlog_<?php echo $post['admitcard_id']?>"
                                        onsubmit="return confirm('Are you sure you want to delete this blog?')">
                                        <a class="btn btn-outline-warning edit-news-btn"
                                            href="edit_admit-card.php?id=<?php echo $post['admitcard_id']; ?>"
                                            data-edit-id="<?php echo $post['admitcard_id']; ?>">Edit Blog</a>
                                        <input type="hidden" name="delete_id"
                                            value="<?php echo $post['admitcard_id']?>">
                                        <button class="btn btn-outline-danger delete-button"
                                            data-delete-id="<?php echo $row['admitcard_id']; ?>"><i
...
$(document).ready(function () {
    $('#searchInput').on('input', function () {
        ...
        $('.searchable-items').each(function () {
            var itemText = $(this).text().toLowerCase(); // Get the text of the item in lowercase
            var item = $(this);
