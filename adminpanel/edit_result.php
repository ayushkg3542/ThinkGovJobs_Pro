...
$query = "SELECT * FROM result_data WHERE result_id = '$result_id'";
...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $result_discription = $_POST["result_discription"];
    $category = $_POST['category'];
    $author_name = $_POST['author_name'];

    // Validate and sanitize the data (you should perform more thorough validation)
    $result_title = trim($result_title);
    $result_discription = trim($result_discription);
    $category = trim($category);
    $author_name = trim($author_name);

    // Prepare and execute the SQL query to insert the data into the database
    $update = "UPDATE result_data SET result_title = '$result_title', result_discription = '$result_discription',category = '$category', author_name = '$author_name' WHERE result_id = '$result_id'";
    $query = mysqli_query($conn, $update);

    if($query){
        echo '<script>alert("Blog Updated Successfully")</script>';
        header('location: answer_key_page.php');
    }else{
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <header>
                            <h1><i class="bi bi-caret-right-fill"></i> New Result Blog</h1>
                        ...
                        <div class="card-body">
                            ...
                            <form id="blog-form" method="post" action="" enctype="multipart/form-data">
                                ...
                                <input type="text" id="title" name="result_title" value="<?= $rows['result_title']?>" required>

                                <label for="author">Author:</label>
                                <input type="text" id="author" name="author_name" value="<?= $rows['author_name']?>" required>

                                <label for="admission">Content:</label>
                                <textarea contenteditable="true" id="editor" name="result_discription" rows="10"
                                    cols="30" required><?= $rows['result_discription']?></textarea>
                                ...
                                <a href="result_page.php" class="btn btn-danger my-3" type="button" name="cancel">Cancel</a>
                            </form>
                        </div>
                    </div>
                </div>
...
<script>
CKEDITOR.replace("result_discription");
</script>
<script>
