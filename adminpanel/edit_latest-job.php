...
$latestjob_id = $_GET['id'];

$query = "SELECT * FROM latestjob_data WHERE latestjob_id = '$latestjob_id'";
...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update = "UPDATE latestjob_data SET latestjob_title = '$latestjob_title', latestjob_discription = '$latestjob_discription',category = '$category', author_name = '$author_name' WHERE latestjob_id = '$latestjob_id'";
    $query = mysqli_query($conn, $update);

    if($query){
        echo '<script>alert("Blog Updated Successfully")</script>';
        header('location: latest_job_page.php');
    }else{
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <header>
                            <h1><i class="bi bi-caret-right-fill"></i> Edit Latest Job Blog</h1>
                        ...
                        <div class="card-body">
                            ...
                            <form id="blog-form" method="post" action="" enctype="multipart/form-data">
                                ...
                                <input type="text" id="title" name="latestjob_title" value="<?= $rows['latestjob_title']?>" required>

                                <label for="author_name">Author Name:</label>
                                <input type="text" id="author_name" name="author_name" value="<?= $rows['author_name']?>" required>

                                <label for="admission">Content:</label>
                                <textarea contenteditable="true" id="editor" name="latestjob_discription" rows="10" cols="30"
                                ...
                                <a href="latest_job_page.php" class="btn btn-danger my-3" type="button" name="cancel">Cancel</a>
                            </form>
                        </div>
                    </div>
                </div>
