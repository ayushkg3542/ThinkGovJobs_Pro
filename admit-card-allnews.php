<?php
 ...
 if (isset($_GET['admitcard_id'])) {
    $blogId = $_GET['admitcard_id']; // Retrieve the blog ID from the query parameter

    // Fetch the full blog content based on the blog ID from the database
    $sql = "SELECT * FROM admitcard_data WHERE admitcard_id = $blogId";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $blogTitle = $row['admitcard_title'];
        $blogDate = $row['date'];
        $blogContent = $row['admitcard_discription'];
    } else {
