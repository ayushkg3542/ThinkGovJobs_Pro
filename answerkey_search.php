<?php
 ...
 $sql = "SELECT * FROM answerkey_data LEFT JOIN blog_category ON answerkey_data.category=blog_category.category_id ORDER BY date DESC limit $offset, $limit";
 ...
 if($result->num_rows > 0){
    while ($row = $result->fetch_assoc()) {
        ...
        $row['answerkey_discription'] = substr($row['answerkey_discription'],0 ,500) . '...';
    ...
    <div class="row my-3">
        <div class="col-sm-8 col-md-8 col-12">
            <div class="card">
                <div class="card-title p-3">
                    <h1>Search result for: <strong><?= $table_search;?></strong></h1>
                </div>
            </div>
            <?php foreach ($blogPosts as $post): ?>
            <div class="card my-3">
                <div class="card-title p-3">
                    <a href="answerkey-allnews.php?answerkey_id=<?php echo $post['answerkey_id']; ?>">
                        <h2><?php echo $post['answerkey_title']; ?></h2>
                    </a>
                    <span><?php echo $post['date']?></span>
                </div>
                <div class="card-body">
                    <div class="card-text">
                        <p><?php echo $post['answerkey_discription']; ?><a
                                href="answerkey-allnews.php?answerkey_id=<?php echo $post['answerkey_id']; ?>">Read
                                More</a></p>
                    </div>
                    <div class="link-part my-5">
                        <p><i class="bi bi-folder-fill"></i> <a href="answerkey.php">Answer Key</a></p>
                        <p><i class="bi bi-chat-left-dots-fill"></i> <a
                                href="answerkey-allnews.php?admission_id=<?php echo $post['answerkey_id']; ?>">Leave a
                                comment</a></p>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
            <?php 
                $pagination ="SELECT * FROM answerkey_data"; 
            ...
            <ul class="pagination pt-2 pb-5">
                ...
                <li class="page-item <?= ($i==$page) ? $active = 'active':'';?>">
                    <a href="answer-key.php?page_no=<?= $i?>" class="page-link"><?= $i?></a>
                </li>
                <?php }?>
            </ul>
        </div>
        <div class="col-sm-4 col-md-4 col-12">
            <div class="card mb-3">
                ...
                ?>
                <form action="answerkey_search.php" method="GET">
            ...
            <div class="card my-3">
                ...
                <?php 
                    $select2 = "SELECT * FROM answerkey_data ORDER BY date DESC";
                ...
                <div class="card-body p-3">
                    <ul>
                        ...
                        <div style="display: flex; gap: 10px;">
                            <a href="./answerkey-allnews.php?answerkey_id=<?php echo $title['answerkey_id']; ?>"
                                style="display: block; text-decoration: none;" class="card-link-wp">
                                <?= $title['answerkey_title'] ?>
                            </a>
                        </div>
                        <hr>
                        <?php $postCounter++;
