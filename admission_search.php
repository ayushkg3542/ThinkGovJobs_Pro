<?php
 ...
 $limit = 5;
 $offset = ($page-1)*$limit;
 $sql = "SELECT * FROM admission_data LEFT JOIN blog_category ON admission_data.category=blog_category.category_id ORDER BY date DESC";
 ...
 if($result->num_rows > 0){
    while ($row = $result->fetch_assoc()) {
        ...
        $row['admission_discription'] = substr($row['admission_discription'],0 ,500) . '...';
...
<div class="container">
    <div class="row my-3">
        <div class="col-sm-8 col-md-8 col-12">
            ...
            <div class="card my-3">
                <div class="card-title p-3">
                    <a href="admission-allnews.php?admission_id=<?php echo $post['admission_id']; ?>">
                        <h2><?php echo $post['admission_name']; ?></h2>
                    </a>
                    <span><?php echo $post['date']?></span>
                </div>
                <div class="card-body">
                    <div class="card-text">
                        <p><?php echo $post['admission_discription']; ?><a
                                href="admission-allnews.php?admission_id=<?php echo $post['admission_id']; ?>">Read
                                More</a></p>
                    </div>
                    <div class="link-part my-5">
                        <p><i class="bi bi-folder-fill"></i> <a href="admission.php">Admission</a></p>
                        <p><i class="bi bi-chat-left-dots-fill"></i> <a
                                href="admission-allnews.php?admission_id=<?php echo $post['admission_id']; ?>">Leave a
                                comment</a></p>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
            <?php 
                $pagination ="SELECT * FROM admission_data"; 
            ...
            <ul class="pagination pt-2 pb-5">
                ...
                <li class="page-item <?= ($i==$page) ? $active = 'active':'';?>">
                    <a href="admission.php?page_no=<?= $i?>" class="page-link"><?= $i?></a>
                </li>
                <?php }?>
            </ul>
        </div>
        <div class="col-sm-4 col-md-4 col-12">
            ...
            <div class="card my-3">
                ...
                <?php 
                    $select2 = "SELECT * FROM admission_data ORDER BY date DESC";
                ...
                <div class="card-body p-3">
                    <ul>
                        ...
                        <div style="display: flex; gap: 10px;">
                            <a href="./admission-allnews.php?admission_id=<?php echo $title['admission_id']; ?>"
                                style="display: block; text-decoration: none;" class="card-link-wp">
                                <?= $title['admission_name'] ?>
                            </a>
                        </div>
                        <hr>
                        <?php 
