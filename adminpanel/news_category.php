<?php 
include('include/header.php');
include('include/navbar.php');
include('include/sidebar.php');
include('../connection.php');

// Check if the delete button is clicked
if (isset($_POST['delete_id'])) {
    $delete_id = mysqli_real_escape_string($conn, $_POST['delete_id']);

    // Perform the delete operation in the database
    $delete_query = "DELETE FROM blog_category WHERE id = '$delete_id'";
    $result = mysqli_query($conn, $delete_query);

    if ($result) {
        // Successful deletion
        echo "<script type='text/javascript'>alert('Category deleted successfully.')</script>";
    } else {
        // Failed deletion
        echo "<script type='text/javascript'>alert('Error: Unable to delete the category.')</script>";
    }
}
?>
<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Blog Category</h1>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="card-tools d-flex">
                                <a href="add_category.php" class="btn btn-outline-secondary"><i class="bi bi-plus"></i>
                                    Add Category</a>
                            </div>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body table-responsive p-0">
                            <table class="table table-hover text-nowrap" id="myTableData">
                                <thead>
                                    <th>Sr. No.</th>
                                    <th>Category Name</th>
                                    <!-- <th>Update</th> -->
                                    <th>Delete</th>
                                    <!-- <th>DELETE</th> -->
                                </thead>
                                <tbody>
                                    <?php
                                    $query = "SELECT * FROM blog_category";
                                    $query_result = mysqli_query($conn, $query);
                                    $rows = mysqli_fetch_all($query_result, MYSQLI_ASSOC);
                                    
                                    // Check if there are rows returned from the database
                                    if ($rows) {
                                        foreach ($rows as $row) {
                                            ?>
                                    <tr>
                                        <td><?php echo $row['id']; ?></td>
                                        <td><?php echo $row['category_name']; ?></td>
                                        <!-- <td><a href="edit_category.php?id="
                                                class="btn btn-outline-success editBtn"><i class="bi bi-pencil-square"></i></button></td> -->
                                        <td>
                                            <form action="" method="post"
                                                onsubmit="return confirm('Are you sure you want to delete this category?');">
                                                <input type="hidden" name="delete_id"
                                                    value="<?php echo $row['id']; ?>">
                                                <button type="submit" class="btn btn-outline-danger deletebtn">
                                                    <i class="nav-icon fas fa-trash"></i>
                                                </button>
                                            </form>
                                        </td>
                                    </tr>
                                    <?php
                                        }
                                    } else {
                                        // No categories found in the database
                                        ?>
                                    <tr>
                                        <td colspan="3">No categories found.</td>
                                    </tr>
                                    <?php
                                    }     
                        ?>
                                </tbody>
                            </table>
                        </div>
                        <!-- /.card-body -->
                    </div>
                    <!-- /.card -->
                </div>
            </div>
        </div>
    </section>
</div>

<?php include('include/footer.php')?>